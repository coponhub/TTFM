...
use super::{apply_arithmetic_op, build_resolved_literal_expr, build_storage_column_expr, build_tag_value_agg_expr};

/// 集約を含まない純粋な算術演算ノードをカラム参照式に変換します（agg_ctx 不要）。
pub(super) fn build_calculation_expr(calc: &ResolvedCalculationNode) -> SimpleExpr {
...
pub(super) fn build_calculation_eav_expr(calc: &ResolvedCalculationNode) -> SimpleExpr {
...
pub(super) fn fold_simple_operand(op: &ResolvedOperand, child_results: Vec<SimpleExpr>) -> Option<SimpleExpr> {
    match op {
        ResolvedOperand::Literal(lab) => {
            let expr = if let Some(bytes) = crate::util::parse_size(&lab.as_str()) {
                sea_query::Expr::val(bytes).cast_as(crate::db::SqlType::DOUBLE).into()
            } else {
                match lab.value() {
                    crate::types::LabelValue::Integer(i) => {
                        sea_query::Expr::val(i).cast_as(crate::db::SqlType::DOUBLE).into()
                    }
                    crate::types::LabelValue::String(s)
                    | crate::types::LabelValue::Literal(s) => sea_query::Expr::val(s.clone()).into(),
                    crate::types::LabelValue::Boolean(b) => sea_query::Expr::val(b).into(),
                    crate::types::LabelValue::Double(bits) => {
                        sea_query::Expr::val(f64::from_bits(bits)).into()
                    }
                    crate::types::LabelValue::Null => sea_query::Expr::val(None::<i32>).into(),
                }
            };
            Some(expr)
        }
