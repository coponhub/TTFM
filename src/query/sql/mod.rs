...
use crate::query::lens_resolver::{
    extract_nvalue_projection_parts, ResolvedAggregationNode, ResolvedNode,
    ResolvedOperand,
};
use crate::query::lens_schema::{to_bin_op, StorageMapping};
use crate::types::{Label, TagType};
use sea_query::{
    Alias, BinOper, Condition, Expr, ExprTrait, Func, Query,
...
fn build_unique_agg(
    ...
    if let Some(ctx) = context {
        sub.and_where(Expr::col(Col::ItemId).in_subquery(ids_from_pick(build_pick_sql(ctx, view))));
    }
    if let Some(filter_node) = cond {
        sub.and_where(Expr::col(Col::ItemId).in_subquery(ids_from_pick(build_pick_sql(&filter_node, view))));
    }
...
fn ids_from_pick(pick: SelectStatement) -> SelectStatement {
    Query::select()
        .column(Col::ItemId)
        .from_subquery(pick, Alias::new("ids_src"))
        .to_owned()
}
...
fn subquery(stmt: SelectStatement) -> SimpleExpr {
...
pub fn build_boolean_sql(node: &ResolvedNode, view: &str) -> SelectStatement {
    ...
    match node {
        ...
        ResolvedNode::AggregationAggregationMatch { left, op, right } => {
            build_direct_boolean_select(
                subquery(build_agg(left, view)),
                *op,
                subquery(build_agg(right, view)),
                view,
            )
        }
