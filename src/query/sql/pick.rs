...
use crate::query::lens_resolver::{
    ResolvedAggregationNode, ResolvedCalculationNode, ResolvedNode, ResolvedOperand,
};
...
use super::{
    ...
    build_resolved_literal_expr,
    build_storage_column_expr, build_tag_value_agg_expr,
    label_to_unit_aware_expr, subquery,
    try_dispatch_common,
    build_nest_match_sql, build_nest_nest_match_sql, build_merged_nest_match_sql,
    needs_aggregation_context, build_aggregation_context,
    needs_nest_context, build_nest_context,
...
fn build_agg_calc_match_nest(
    ...
    let calc_expr = if calc.contains_aggregation() {
        build_agg_calc_subquery_nest(calc, view, agg_ctx, nest_ctx)
    } else {
        build_agg_calc_expr(calc, agg_ctx)
    };
    stmt.cond_where(Expr::expr(agg_expr).binary(to_bin_op(op), calc_expr));
