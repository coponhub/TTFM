...
## 4. 開発ワークフロー（AI アシスタント用）

### 一時ファイルを用いた事前検証
コードの生成や修正を行う際は、**原則として**以下の手順を遵守してください。
（変数名の変更や誤字修正など、ごく軽微な修正はこの限りではありませんが、迷った場合はこの手順に従ってください）

1.  **サンドボックスへの書き出し**: `src/gemini_sandbox.rs` という一時ファイルを作成し、そこに新しいコードや修正後の実装を書き出します。
2.  **コンパイル・チェック**: `cargo check` 等を実行し、生成されたコードにコンパイルエラーがないことを確認します。
3.  **レビュー依頼**: 検証を通過した後、ユーザーにコードを提示し、対象ファイルへの反映許可を求めてください。

これにより、既存のソースコードを破壊することなく、安全かつ高品質なコード変更を維持します。
なお、`src/gemini_sandbox.rs` は Git の管理対象外（`.gitignore`）に設定されています。