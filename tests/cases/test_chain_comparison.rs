...
fn test_chain_comparison_logic() -> anyhow::Result<()> {
    ...
    assert!(!names.contains(&"small.txt"), "Should NOT contain small.txt");
    assert!(!names.contains(&"large.txt"), "Should NOT contain large.txt");

    // クエリ: 10 :<= size: :< 1001
    // 全てヒットすべき
    let result_all = fm.search("10 :<= size: :< 1001", Default::default())?;
    let all_names: Vec<_> = result_all.results.iter().map(|r| r.name.as_str()).collect();
    ...
    let rev_names: Vec<_> = result_rev.results.iter().map(|r| r.name.as_str()).collect();
    ...
    let result_agg = fm.search("sum((100 :< size: :<= 500) & size:)", Default::default())?;
    assert_eq!(result_agg.scalar, Some(200.0), "Sum of medium file size should be 200");
