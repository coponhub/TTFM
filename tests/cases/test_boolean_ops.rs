...
fn test_boolean_arithmetic_ops() -> anyhow::Result<()> {
    ...
    let all_items = fm.search("", Default::default())?;
    let files_only: Vec<_> = all_items.results.iter()
        .filter(|r| r.item_kind == ttfm::ItemKind::File && r.name != "data")
        .collect();
    assert_eq!(files_only.len(), 3, "Total file items should be 3. Found: {:?}", files_only.iter().map(|r| &r.name).collect::<Vec<_>>());
    ...
    assert_eq!(res_sum.results[0].name, "2", "sum(is_dir:) should be 2 (data and subdir)");

    // 1b. フィルタ付きの sum 集計
    let query_sum_filter = "sum(name:subdir & is_dir:)";
    let res_sum_filter = fm.search(query_sum_filter, Default::default())?;
    assert_eq!(res_sum_filter.results[0].name, "1", "sum(name:subdir & is_dir:) should be 1");
    ...
    assert_eq!(res_calc.results[0].name, "6", "sum(is_dir: + 1) should be 6. Found: {}", res_calc.results[0].name);

    // 3. Boolean 同士の比較 (is_dir:true)
    let query_cmp = "is_dir:true";
    let res_cmp = fm.search(query_cmp, Default::default())?;
    // data と subdir がヒットするはずなので 2
    assert_eq!(res_cmp.results.len(), 2, "2 directories should match. Found: {:?}", res_cmp.results.iter().map(|r| &r.name).collect::<Vec<_>>());
