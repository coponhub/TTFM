...
fn test_metadata_error_recovery_integration() {
    ...
    // 3. 救済された値を検索して検証
    // 数値型の救済値 (-1) で検索
    let results = fm.search("size:-1").expect("Search for size:-1 should succeed");
    
    // 検証: loop_link が救済されてヒットするはず
    assert_eq!(results.len(), 1, "Should find exactly one file with metadata error");
    assert!(results[0].primary_value().unwrap().contains("loop_link"));
    
    // 文字列表現の救済値 ("-") も検証
    let results_str = fm.search("size_str:-").expect("Search for size_str:- should succeed");
