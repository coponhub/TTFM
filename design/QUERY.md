...
- **データ構造同士の関係性**
    - **集約による平坦化 (Lv. 1, 2, n → Lv. 0, n-1)**: 集約関数 (`count`, `sum` 等) を適用すると、構造が平坦化される。
        - **Lv. 1 (フラットリスト) → Lv. 0 (スカラー)**: リストに対して集約を行い、単一の値にする。 (例: `sum(size:)`)
        - **Lv. n (ネスト) → Lv. n-1**: ネストの最も深いプロジェクションを集約し、評価値(`nvalue`)に変換する事でネストの深度が1つ浅くなる。 (例: `sum(project: &: size:)` → `project:` のみに)
    - **プロジェクションとネストの深化 (Lv. 1 → Lv. 2 → Lv. n)**
        - **Lv. 1 → Lv. 2**: `Type:` を指定することで、アイテムのフラットリストを特定のラベルをキーとしたプロジェクション(グループ)に変換する。
        - **Lv. 2 → Lv. n**: プロジェクション同士を `&:` 演算子で結合すると、複合キーのネストとなり階層が深くなる。 (例: `project: &: extension:`)
    - **比較による要素の展開 (Lv. 2, n → Lv. 1)**: プロジェクションやネストに対して、ラベル比較 (`:>`, `:=` 等) を行うと、条件を満たすグループに抽出され、そのグループに属するアイテムのフラットリスト (Lv. 1) として展開される。
        - 例: `parentdir: &: count(extension:jpg) :> 10`
    - **評価値 (nvalue) の付与**: スカラー (Lv. 0) またはスカラーを返す演算・集約は、`&:` 演算子を通じてプロジェクション (Lv. 2) や ネスト (Lv. n) に対して評価値 (`nvalue`) として付与される。この時、階層の深さは変化しない。
- **データ構造同士の演算**
    ...
    - **算術演算 (`+`, `-`, `*`, `/`, `%`) と ラベル比較 (`:=`, `:>` 等)**: スカラー(Lv.0)、プロジェクション(Lv.2)、ネスト(Lv.n) の間で可能。**フラットリスト(Lv.1) に対しては適用不可**。
        - **対 スカラー (Lv. 0)**: どのデータ構造に対しても演算・比較が可能。
        - **プロジェクション (Lv. 2) / ネスト (Lv. n) 同士**:
            - キー構造が一致している場合: グループ単位での評価値(`nvalue`)等の計算・比較が可能。 (例: `(parentdir: &: sum(size:)) := (parentdir: &: avg(size:))`)
            - キー構造が異なる場合: グループの意味合いが衝突するため**エラー**となる。
            - 行レベルの演算: 個々のプロジェクション同士 (例: `width: * height:`) であれば、同アイテム内で行レベルとして計算される。
