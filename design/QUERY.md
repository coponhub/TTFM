...
- **Eval (`@()`)**:
    - 形式: `@([query])`
    - **意味**: 括弧内クエリ `Q` を評価し、その**各結果要素を「その要素を指すクエリ項」へ翻訳**して `|` (和集合) で連結した**クエリ**に展開する。展開結果は通常のクエリとして外側の文脈で評価され、他の演算と自由に合成できる (例: `@(milestone:m1) & extension:rs`)。
    - **動機**: 定義アイテム (tag/type/label) に付与したメタは、`milestone:m1` のように検索しても**定義アイテム**しか引けず、そのタグを実際に持つアイテム群へ橋渡しする手段が無い。Eval はこの一段 (定義 → そのタグを持つアイテム) を埋めるプリミティブであり、メタによるタグ選択・階層/含意のロールアップ・エイリアスを同一の仕組みで表現できる。
    - **翻訳規則** (結果要素の種類でディスパッチする):

        | `Q` の結果要素 | 翻訳される項 |
        |---|---|
        | tag 定義 (item_kind=tag, content=`type:label`) | `type:label` (そのタグを持つアイテム) |
        | type 定義 (content=`type`) | `type:` |
        | label 定義 (content=`label`) | `*:label` |
    ...
    - **非対称性**: 定義アイテム (tag/type/label) は「指示対象 = そのタグを持つアイテム」へ deref されるのに対し、具体アイテムは「自分自身」が指示対象なので `item_id` で自己参照する。これにより `@(milestone:m1)` は milestone:m1 を持つタグ定義群を、そのタグを持つアイテムへ展開できる。
    - **例**:
        - `@(milestone:m1)` → `project:ttfm-cli | project:ttfm-lib` (milestone:m1 が付与されたタグを持つアイテム)
        - `@(project: &: extension:)` → `(project:A & extension:rs) | (project:B & extension:rs) | …`
        - `@(project:)` → `project:A | project:B | …`

## 2.  **評価の優先順位**:
- 以下の順序で評価される。
-  `ネスト` > `密着ラベル比較 / TypedTag` > `汎用ラベル比較` > `ラベル取得` > `集約` > `算術演算` > `集合演算 `
- **注**: `集約` や `ネスト`、`算術演算`、`Eval (@())` 等で使用される括弧 `()` 内の式は、再帰的に評価され、常に括弧の外側の演算よりも優先される。
