...
- **Eval (`q()`)**:
    ...
    - **動機**: 定義アイテム (tag/type) に付与したメタは、`milestone:m1` のように検索しても**定義アイテム**しか引けず、そのタグを実際に持つアイテム群へ橋渡しする手段が無い。Eval はこの一段 (定義 → そのタグを持つアイテム) を埋めるプリミティブであり、メタによるタグ選択・階層/含意のロールアップ・エイリアスを同一の仕組みで表現できる。
