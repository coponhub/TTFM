...
### 2.2 参照中心設計 (Item/Reference-Centric)
システムが管理する全ての参照は **Item（アイテム）** と定義し、一意のIDを持つ。アイテムは以下の2つに分類される。
...
4.  **Fetching Phase (OneViewからのデータ取得)**:
    ...
    - **種類** Fetcherは返り値の種類に応じて以下通り用意する
        - **Item (TypeGroup)** Type(カラム)の組み合わせ毎に分類したItem
        - **Projection (LabelGroup)**: ラベル毎に分類したItem
        - **Computation**: 各スカラー型による仮想Itemのテーブルを返す
            - 単純な集約、スカラー比較、算術演算など、実際の「ItemやLabel」を返さないもの
        - **FlatTable**: キャッシュ等で使用する、シンプルなテーブル
