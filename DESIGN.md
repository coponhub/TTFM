...
1.  **クエリ解析**: 検索クエリをパーサによって AST（抽象構文木）へ変換。
    - クエリはTypedTag (`key:label`) および論理式 (`&`=積集合, `|`=和集合, `-`=差集合, `()`, `""`) を受け付ける。
    - ラベル演算子:「!, >, <, >=, <=」を:の後、Labelの前にに記載することでLabelに対する演算子として使える
        - 例: extension:!log で.log以外のアイテムにマッチ
	- 例: mtime:>2024/1/1 でmtimeが2024/1/1 のアイテムにマッチ 
...
2.  **論理演算の解決**: 各 `TypedTag` に対して条件式を生成。`oneview` に対してクエリを発行し、取得。
3.  **実行**: 一致する ID を集約して結果を返す。結果は `rank` の降順でソートされる。
